...
class HarnessAgent:
    ...
    def step(self, prompt_or_message):
        from .response import HarnessBaseMessage, HarnessResponse
    ...
    def _call_claude(self, prompt):
        ...
        try:
            cmd = f'cat "{prompt_file}" | claude -p --output-format text --no-input 2>/dev/null'
            ...
            raise RuntimeError(
                "Claude CLI ('claude') not found. Install it:\n"
                "  npm install -g @anthropic-ai/claude-code\n"
                "Or use OpenCode: https://github.com/opencode-ai/opencode"
            ) from error
        except subprocess.TimeoutExpired as error:
            raise RuntimeError("Claude CLI timed out after 600 seconds") from error
        finally:
