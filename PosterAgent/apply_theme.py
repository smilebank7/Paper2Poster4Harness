...
def poster_apply_theme(args, actor_config, critic_config):
    ...
    non_overlap_ckpt = pkl.load(
        open(
            f"checkpoints/{args.model_name}_{args.poster_name}_non_overlap_ckpt_{args.index}.pkl",
            "rb",
        )
    )
    ...
    title_actor_agent = HarnessAgent(
    ...
    section_actor_agent = HarnessAgent(
    ...
    for aspect, prompt_types in theme_aspects.items():
        for prompt_type in prompt_types:
            ...
            with open(
                f"prompt_templates/theme_templates/theme_{prompt_type}.txt", "r"
            ) as f:
                prompt = f.read()
    ...
    for key, val in outline.items():
        ...
        if not "subsections" in val:
            outline_skeleton[key] = {"section": key}
        else:
            for subsection_name, subsection_dict in val["subsections"].items():
                outline_skeleton[subsection_dict["name"]] = {"section": key}
    ...
    for style in ["background", "title"]:
        for sub_style in theme_styles[style].keys():
            ...
            log = apply_theme(
                title_actor_agent, actor_prompt, args.max_retry, existing_code=""
            )
            ...
            shutil.copy(
                "poster.pptx",
                f"tmp/theme_poster_<{sections[0]}>_<{style}>_<{sub_style}>.pptx",
            )
    ...
    section_actor_template = jinja_env.from_string(
        section_theme_actor_config["template"]
    )
    ...
    for style_dict in outline_skeleton_list:
        ...
        jinja_args = {
            "style_json": json.dumps(
                {curr_subsection: style_dict[curr_subsection]["style"]}, indent=4
            ),
            "function_docs": documentation,
        ...
        shutil.copy(
            "poster.pptx", f"tmp/theme_poster_<{curr_section}>_<{curr_subsection}>.pptx"
        )
    ...
    pkl.dump(
        ckpt,
        open(f"checkpoints/{args.model_name}_{args.poster_name}_theme_ckpt.pkl", "wb"),
    )
