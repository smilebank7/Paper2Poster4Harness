...
from harness.response import HarnessBaseMessage
...
def compute_tp(raw_content_json):
    ...
    for i in range(len(raw_content_json["sections"])):
        raw_content_json["sections"][i]["tp"] = (
            len(raw_content_json["sections"][i]["content"]) / total_length
        )
        raw_content_json["sections"][i]["text_len"] = len(
            raw_content_json["sections"][i]["content"]
        )
...
def get_outline_location(outline, subsection=False):
    ...
    for k, v in outline.items():
        ...
        if subsection:
            if "subsections" in v:
                outline_location[k]["subsections"] = get_outline_location(
                    v["subsections"]
                )
...
def apply_outline_location(outline, location, subsection=False):
    ...
    for k, v in outline.items():
        ...
        if subsection:
            if "subsections" in v:
                new_outline[k]["subsections"] = apply_outline_location(
                    v["subsections"], location[k]["subsections"]
                )
    ...
    for k, v in outline_no_name.items():
        ...
        if "subsections" in v:
            for k_sub, v_sub in v["subsections"].items():
                new_outline[k]["subsections"][k_sub]["name"] = outline[k][
                    "subsections"
                ][k_sub]["name"]
...
def filter_image_table(args, filter_config):
    ...
    with open(
        f"utils/prompt_templates/{agent_filter}.yaml", "r", encoding="utf-8"
    ) as f:
    ...
    filter_actor_agent = HarnessAgent(
        system_message=filter_actor_sys_msg,
        model=filter_config.get("model_type"),
    ...
    json.dump(
        image_information,
        open(
            f"<{args.model_name_t}_{args.model_name_v}>_images_and_tables/{args.poster_name}_images_filtered.json",
            "w",
        ),
        indent=4,
    )
    json.dump(
        table_information,
        open(
            f"<{args.model_name_t}_{args.model_name_v}>_images_and_tables/{args.poster_name}_tables_filtered.json",
            "w",
        ),
        indent=4,
    )
...
def gen_outline_layout_v2(args, actor_config):
    ...
    filtered_table_information = json.load(
        open(
            f"<{args.model_name_t}_{args.model_name_v}>_images_and_tables/{args.poster_name}_tables_filtered.json",
            "r",
        )
    )
    filtered_image_information = json.load(
        open(
            f"<{args.model_name_t}_{args.model_name_v}>_images_and_tables/{args.poster_name}_images_filtered.json",
            "r",
        )
    ...
    planner_agent = HarnessAgent(
        system_message=planner_config["system_prompt"],
        model=actor_config.get("model_type"),
        message_window_size=10,
    )
...
def gen_outline_layout(args, actor_config, critic_config):
    ...
    jinja_env = Environment(undefined=StrictUndefined)
    outline_file_path = (
        f"outlines/{args.model_name}_{args.poster_name}_outline_{args.index}.json"
    )
    ...
    doc_json = json.load(
        open(f"contents/{args.model_name}_{args.poster_name}_raw_content.json", "r")
    )
    filtered_table_information = json.load(
        open(f"images_and_tables/{args.poster_name}_tables_filtered.json", "r")
    )
    filtered_image_information = json.load(
        open(f"images_and_tables/{args.poster_name}_images_filtered.json", "r")
    )
    ...
    with open(
        f"utils/prompt_templates/{agent_init_name}.yaml", "r", encoding="utf-8"
    ) as f:
        config_init = yaml.safe_load(f)

    with open(
        f"utils/prompt_templates/{agent_new_section_name}.yaml", "r", encoding="utf-8"
    ) as f:
        config_new_section = yaml.safe_load(f)

    with open(
        f"utils/prompt_templates/{h1_critic_name}.yaml", "r", encoding="utf-8"
    ) as f:
        config_h1_critic = yaml.safe_load(f)

    with open(
        f"utils/prompt_templates/{h2_actor_name}.yaml", "r", encoding="utf-8"
    ) as f:
    ...
    new_section_actor_agent = HarnessAgent(
    ...
    h1_critic_agent = HarnessAgent(
        system_message=h1_critic_sys_msg,
        model=critic_config.get("model_type"),
    ...
    while True:
        ...
        init_jinja_args = {"json_outline": init_outline, "function_docs": documentation}
        ...
        for section_index in range(1, len(sections)):
            ...
            logs[curr_section] = gen_layout(
                ...
                name_to_hierarchy,
                visual_identifier=curr_section,
                existing_code=logs[sections[section_index - 1]][-1][
                    "concatenated_code"
                ],
            )
            if logs[curr_section][-1]["error"] is not None:
                raise ValueError(
                    f"Failed to generate layout for section {curr_section}."
                )

            layout_cumulative_input_token += logs[curr_section][-1][
                "cumulative_tokens"
            ][0]
            layout_cumulative_output_token += logs[curr_section][-1][
                "cumulative_tokens"
            ][1]

        consumption_log["h1_actor"].append(
            (layout_cumulative_input_token, layout_cumulative_output_token)
        )
        ...
        h1_critic_msg = HarnessBaseMessage.make_user_message(
            role_name="User",
    ...
    for section in sections:
        while True:
            ...
            is_valid, revised = validate_and_adjust_subsections(
                sec_bbox, subsection_location_dict
            )
            if not is_valid:
                ...
                outline_no_sub_locations = fill_location(
                    outline_no_sub_locations, section, revised
                )
            else:
                outline_no_sub_locations = fill_location(
                    outline_no_sub_locations, section, subsection_location
                )
            break

    consumption_log["h2_actor"].append(
        (h2_cumulative_input_token, h2_cumulative_output_token)
    )
    ...
    outline_no_sub_locations_with_name = recover_name_and_location(
        outline_no_sub_locations, new_outline
    )
    ...
    for section_index in range(1, len(sections)):
        ...
        gen_layout_cumulative_input_token += logs[curr_section][-1][
            "cumulative_tokens"
        ][0]
        gen_layout_cumulative_output_token += logs[curr_section][-1][
            "cumulative_tokens"
        ][1]

    consumption_log["gen_layout"].append(
        (gen_layout_cumulative_input_token, gen_layout_cumulative_output_token)
    )
    ...
    with open(
        f"checkpoints/{args.model_name}_{args.poster_name}_ckpt_{args.index}.pkl", "wb"
    ) as f:
        pkl.dump(ckpt, f)
