...
def main():
    ...
    parser.add_argument(
        "--ablation_no_example", action="store_true", help="Ablation study: no example"
    )
    ...
    print(
        f"Content generation token consumption T: {input_token_t} -> {output_token_t}"
    )
    print(
        f"Content generation token consumption V: {input_token_v} -> {output_token_v}"
    )
    ...
    theme_with_alignment = create_theme_with_alignment(
        base_theme, getattr(args, "section_title_vertical_align", None)
    )
    ...
    print(
        f"Total vision model tokens: {total_input_tokens_v} -> {total_output_tokens_v}"
    )
