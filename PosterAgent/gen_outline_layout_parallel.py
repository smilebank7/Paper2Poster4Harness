...
logging.basicConfig(
    level=logging.DEBUG, format="%(threadName)s: %(message)s", stream=sys.stdout
)
...
def layout_process_section(
    ...
    if section_index == 0:
        new_section_outline = {
            "meta": new_outline["meta"],
            curr_section: new_outline[curr_section],
        }
    ...
    existing_code = (
        ""  # Or fetch from a stable location that is not dependent on real-time results
    )

    # Call gen_layout
    section_logs = gen_layout_parallel(
        create_agent_fn(sys_msg, agent_model, window_size),
        new_section_prompt,
        max_retry,
...
def filter_image_table(args, filter_config):
    ...
    json.dump(
        images,
        open(f"images_and_tables/{args.poster_name}_images_filtered.json", "w"),
        indent=4,
    )
    json.dump(
        tables,
        open(f"images_and_tables/{args.poster_name}_tables_filtered.json", "w"),
        indent=4,
    )
...
def gen_outline_layout(args, actor_config, critic_config):
    ...
    while True:
        ...
        logs, layout_cumulative_input_token, layout_cumulative_output_token = (
            layout_process_section_wrapped(
                sections,
                new_outline,
                init_template,
                new_section_template,
...
"""
    ...
    for section, final_sub_loc in updated_sections:
        outline_no_sub_locations = fill_location(
            outline_no_sub_locations, section, final_sub_loc
        )
