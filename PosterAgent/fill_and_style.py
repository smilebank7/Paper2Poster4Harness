...
def fill_poster_content(args, actor_config):
    ...
    ckpt = pkl.load(
        open(
            f"checkpoints/{args.model_name}_{args.poster_name}_ckpt_{args.index}.pkl",
            "rb",
        )
    )
    logs = ckpt["logs"]
    ...
    for section_index in range(len(sections)):
        ...
        if section_index == 0:
            ...
            existing_code = content_logs[sections[section_index - 1]][-1][
                "concatenated_code"
            ]
        content_logs[section_name] = fill_content(actor_agent, prompt, 3, existing_code)

        shutil.copy("poster.pptx", f"tmp/content_poster_<{section_name}>.pptx")

        if content_logs[section_name][-1]["error"] is not None:
            raise Exception(
                f"Error in filling content for {section_name}: {content_logs[section_name][-1]['error']}"
            )
    ...
    pkl.dump(
        ckpt,
        open(
            f"checkpoints/{args.model_name}_{args.poster_name}_content_ckpt_{args.index}.pkl",
            "wb",
        ),
    )
...
def stylize_poster(args, actor_config):
    ...
    for section_index in range(len(sections)):
        ...
        if section_index == 0:
            ...
            existing_code = style_logs[sections[section_index - 1]][-1][
                "concatenated_code"
            ]
        style_logs[section_name] = stylize(
            actor_agent, prompt, args.max_retry, existing_code
        )
    ...
    with open(
        f"checkpoints/{args.model_name}_{args.poster_name}_style_ckpt_{args.index}.pkl",
        "wb",
    ) as f:
        pkl.dump(ckpt, f)
...
if __name__ == "__main__":
    ...
    fill_total_input_token, fill_total_output_token = fill_poster_content(
        args, actor_config
    )
    style_total_input_token, style_total_output_token = stylize_poster(
        args, actor_config
    )
