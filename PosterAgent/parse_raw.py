...
def parse_raw(args, actor_config, version=1):
    ...
    if len(text_content) < 500:
        ...
        try:
            ...
            model_utils = importlib.import_module("utils.src.model_utils")

            parser_model = marker_models.create_model_dict(
                device="cuda", dtype=torch_module.float16
            )
            text_content, _ = model_utils.parse_pdf(
                raw_source, model_lst=parser_model, save_file=False
            )
        except Exception:
            print("Marker fallback unavailable, continuing with docling output.")
    ...
    for section in content_json["sections"]:
        if type(section) != dict or not "title" in section or not "content" in section:
            print(
                f"Ouch! The response is invalid, the LLM is not following the format :("
            )
            print("Trying again...")
            raise
...
def gen_image_and_table(args, conv_res):
    ...
    output_dir = Path(
        f"<{args.model_name_t}_{args.model_name_v}>_images_and_tables/{args.poster_name}"
    )
...
if __name__ == "__main__":
    ...
    # Parse raw content
    input_token, output_token, conversion_result = parse_raw(args, agent_config)

    # Generate images and tables
    _, _, _, _ = gen_image_and_table(args, conversion_result)
