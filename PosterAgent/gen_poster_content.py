...
from harness.vision import heuristic_overflow_check
...
def gen_content_process_section(
    ...
    while True:
        ...
        new_section_outline, suggested = generate_length_suggestions(
            result_json, json.dumps(outline[section_name]), raw_outline[section_name]
        )
...
def gen_content_parallel_process_sections(
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in as_completed(futures):
            section_name, result_json, sec_input_token, sec_output_token = (
                future.result()
            )
...
def gen_poster_title_content(args, actor_config):
    ...
    jinja_args = {
        ...
        "title_font_size": getattr(args, "poster_title_font_size", None)
        or getattr(args, "title_font_size", None),
        "author_font_size": getattr(args, "poster_author_font_size", None)
        or getattr(args, "author_font_size", None),
    }
    prompt = template.render(**jinja_args)
    # Step the actor_agent and track tokens
    actor_agent.reset()
...
def gen_bullet_point_content(
    args, actor_config, critic_config, agent_modify=True, tmp_dir="tmp"
):
    ...
    jinja_env = Environment(undefined=StrictUndefined)
    template = jinja_env.from_string(content_config["template"])
    jinja_env.from_string(critic_content_config["template"])
    ...
    for k in textboxes_by_panel:
        textboxes_by_panel[k] = sorted(
            textboxes_by_panel[k], key=lambda x: x.get("textbox_id", 0)
        )
    ...
    def _process_section(i):
        ...
        actor_agent = HarnessAgent(
            system_message=content_config["system_prompt"],
            model=actor_config.get("model_type"),
            message_window_size=30,
        )
        ...
        while args.estimate_chars:
            ...
            try:
                ...
                for j in range(num_textboxes):
                    ...
                    total_bullet_length += compute_bullet_length(
                        result_json[bullet_content_key]
                    )
            except Exception:
                result_json = old_result_json
                break
            ...
            if total_bullet_length > total_expected_length:
                percentage_to_shrink = int(
                    (total_bullet_length - total_expected_length)
                    / total_bullet_length
                    * 100
                )
                percentage_to_shrink = min(90, percentage_to_shrink + 10)
                old_result_json = copy.deepcopy(result_json)
                response = actor_agent.step(
                    "Too long, please shorten the bullet points by "
                    + str(percentage_to_shrink)
                    + "%."
                )
                t_in, t_out = account_token(response)
                local_t_in += t_in
        ...
        # Overflow/blank detection & correction
        for j, text_arrangement in enumerate(target_textboxes[:num_textboxes]):
            ...
            while True:
                if args.ablation_no_commenter:
                    break
                curr_round += 1
                decision = heuristic_overflow_check(
                    result_json[bullet_content],
                    text_arrangement["width"],
                    text_arrangement["height"],
                    font_size_pt=args.bullet_font_size,
                )
                if decision == "1":
                    if curr_round > 10:
                        print(
                            f"Section {i}: Too many rounds of modification, breaking..."
                        )
                        break
                    if agent_modify:
    ...
    title_json, title_input_token, title_output_token = gen_poster_title_content(
        args, actor_config
    )
    ...
    return (
        total_input_token_t,
        total_output_token_t,
        total_input_token_v,
        total_output_token_v,
    )
...
def gen_poster_content(args, actor_config):
    ...
    if not isinstance(outline, dict):
        raise RuntimeError("Invalid outline format")
    ...
    poster_content, total_input_token, total_output_token = (
        gen_content_parallel_process_sections(
            sections,
            outline,
            raw_content,
            raw_outline,
    ...
    json.dump(
        poster_content,
        open(
            f"contents/{args.model_name}_{args.poster_name}_poster_content_{args.index}.json",
            "w",
        ),
        indent=2,
    )
