...
def get_agent_config(model_type="harness"):
    return {"model_type": "harness", "model_platform": "claude_cli"}
...
def run_modular(all_code, file_name, with_border=True, with_label=True):
    ...
    if with_border and with_label:
        ...
        concatenated_code += save_helper_info_border_label.format(
...
def edit_modular(
    ...
    for attempt in range(num_retries + 1):
        ...
        concatenated_code, output, error = run_modular(
            all_code_changed, file_name, False, False
        )
        log.append(
            {
                "code": new_code,
...
def add_border_to_all_elements(
    prs, border_color=RGBColor(255, 0, 0), border_width=Pt(2)
):
    """
...
def add_border_and_labels(
    ...
    """
    label_diameter_emu = pt_to_emu(
        label_diameter_pt
    )  # convert diameter (points) to EMUs
        ...
        for shape in slide.shapes:
            ...
            try:
                ...
                labeled_elements[label_counter] = {
                    ...
                    "font_size": f"{shape.text_frame.font.size} PT"
                ...
                print(
                    f"Could not add border/label to shape (type={shape.shape_type}): {e}"
                )
...
def check_bounding_boxes(bboxes, overall_width, overall_height):
    ...
    def boxes_overlap(box_a, box_b):
        ...
        # there's no overlap.
        no_overlap = (
            right_a <= left_b  # A is completely left of B
            or right_b <= left_a  # B is completely left of A
            or bottom_a <= top_b  # A is completely above B
            or bottom_b <= top_a
...
def is_poster_filled(
    ...
    if (
        leftover_left > max_lr_margin
        or leftover_right > max_lr_margin
        or leftover_top > max_tb_margin
        or leftover_bottom > max_tb_margin
    ):
        return False
...
async def rendered_dims(html: Path) -> tuple[int, int]:
    ...
    async with playwright_async() as p:
        ...
        body_box = await page.eval_on_selector(
            "body", "el => el.getBoundingClientRect()"
        )
...
def actual_rendered_length(
    text, width_in_inches, height_in_inches, font_size_points, line_spacing_points=None
):
    """
...
def generate_length_suggestions(
...
def get_snapshot_from_section(
    leaf_section,
    section_name,
    name_to_hierarchy,
    leaf_name,
    section_code,
    empty_poster_path="poster.pptx",
):
    hierarchy = name_to_hierarchy[leaf_name]
    hierarchy_overflow_name = (
        f"tmp/overflow_check_<{section_name}>_<{leaf_section}>_hierarchy_{hierarchy}"
    )
    ...
    ppt_to_images(f"{hierarchy_overflow_name}.pptx", hierarchy_overflow_name, dpi=200)
    ...
    locations = convert_pptx_bboxes_json_to_image_json(
        curr_location, poster_width, poster_height
    )
    zoomed_in_img = zoom_in_image_by_bbox(
        poster_image, locations[leaf_name], padding=0.01
    )
