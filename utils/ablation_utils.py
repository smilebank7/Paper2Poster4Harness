...
def no_tree_get_layout(poster_width, poster_height, panels, figures, agent_config):
    ...
    planner_agent = HarnessAgent(
        ...
        model=agent_config.get("model_type"),
    ...
    while True:
        ...
        if (
            not "panel_arrangement" in arrangements
            or not "figure_arrangement" in arrangements
            or not "text_arrangement" in arrangements
        ):
            print("Error: Invalid response, retrying...")
            continue

        if len(arrangements["panel_arrangement"]) != len(panels) or len(
            arrangements["figure_arrangement"]
        ) != len(figures):
            print("Error: Invalid response, retrying...")
            continue
        break
    return (
        arrangements["panel_arrangement"],
        arrangements["figure_arrangement"],
        arrangements["text_arrangement"],
        input_token,
        output_token,
    )
