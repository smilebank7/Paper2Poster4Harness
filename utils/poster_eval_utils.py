...
def md_to_blocks(md: str, base_dir=""):
    blocks, pos = [], 0
    ...
    for m in pat.finditer(md):
        ...
        blocks.append(
            {
                "type": "image_url",
                "image_url": {"url": pil_to_data_uri(Image.open(img_path), fmt="PNG")},
            }
        )
        pos = m.end()
...
def convert_folder_to_grayscale(input_folder, output_folder):
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith((".jpg", ".jpeg", ".png")):
            ...
            img = (
                Image.open(input_path).convert("L").convert("RGB")
            )  # grayscale + 3 channels
            img.save(output_path)


def compute_fid_with_grayscale(
    reference_poster_folder, generated_poster_img_folder, clip=False
):
...
def compute_fid(reference_poster_folder, generated_poster_img_folder, clip=False):
    if clip:
        return compare_folders_with_clip(
            reference_poster_folder, generated_poster_img_folder
        )
    model = fid.InceptionV3([fid.InceptionV3.BLOCK_INDEX_BY_DIM[2048]]).to("cuda")

    m1, s1 = compute_statistics_of_path(reference_poster_folder, model, 1, 2048, "cuda")
    m2, s2 = compute_statistics_of_path(
        generated_poster_img_folder, model, 1, 2048, "cuda"
    )
...
def get_poster_text(poster_path, check_fail=True):
    ...
    if len(text_content) < 500 and check_fail:
        ...
        text_content, rendered = parse_pdf(
            poster_path, model_lst=parser_model, save_file=False
        )
    return text_content
...
def png_to_optimized_jpeg(
    img: Image.Image, max_size=(2048, 2048), quality=80
) -> BytesIO:
    """
...
def get_answers_and_remove_answers(questions):
    ...
    for key, val in questions.items():
        question_only[key] = {"question": val["question"], "options": val["options"]}
...
def eval_qa_get_answer(
    ...
    if input_type == "text":
        ...
        msg = HarnessBaseMessage.make_user_message(
            role_name="User",
            content=prompt,
...
def compute_accuracy(predicted, ground_truth, aspects):
    ...
    for aspect, total in total_by_aspect.items():
        ...
        aspect_summary[aspect] = {"total": total, "correct": correct, "accuracy": acc}
...
def gen_eval_markdown(paper_name, poster_method, poster_path, figure_count_only=False):
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".pdf", prefix="poster_", dir="tmp", delete=False
    ) as tf:
        unique_pdf = tf.name
    ...
    markdown = conv_res.document.save_as_markdown(
    ...
    for res in images_and_text:
        image_path = os.path.join(
            "eval_poster_markdown", paper_name, poster_method, res["image_path"]
        )
        ...
        section_text_clip_embedding = compute_clip_embedding(
            section_text, model, processor
        )
        msg = HarnessBaseMessage.make_user_message(
            role_name="User",
            content=prompt,
    ...
    pkl.dump(
        images,
        open(f"eval_poster_markdown/{paper_name}/{poster_method}/images.pkl", "wb"),
    )
    with open(
        f"eval_poster_markdown/{paper_name}/{poster_method}/markdown_with_images.md",
        "w",
    ) as f:
        f.write(new_md)
...
def eval_vlm_as_judge_aspect(poster_image_list, agent_config, eval_aspect):
    ...
    judge_message = HarnessBaseMessage.make_user_message(
        role_name="User",
        content=prompt,
...
def eval_vlm_as_judge(poster_image_list, agent_config, aspect=None):
    ...
    for aspect in aspects:
        results[aspect] = eval_vlm_as_judge_aspect(
            poster_image_list, agent_config, aspect
        )
